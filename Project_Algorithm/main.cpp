...
int main(int argc, const char * argv[]) {
    ...
    Header header("uniprot_sprot.fasta.phr");
    Swipe swipe;
    ...
    chrono::time_point<chrono::system_clock> start, end;
    start = std::chrono::system_clock::now();
    for (int g = 116900; g<123000; g++) {//1878 pbm //offsets.get_size()
    ...
    for(itr = score_max.begin(); itr != score_max.end(); ++itr){
        ...
        //long off = offsets.get_head_offset(itr->second-1);
        //header.acquiert(off);
        //header.getData();
    }
    //header.close_fichier();
    return 0;
}
